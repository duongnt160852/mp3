			...
			<div class="inner-content">
				<div class="music-left">
					...
					<div style="height: 40px;display:flex;background-color: #f5f5f5" class="aaa">
						...
						<div style="width: 100px;padding:10px;text-align: center;">
							<a href="search/album/{{$str}}" style="color:#2e9afe!important;">Album</a>
						</div>
						<div style="width: 1px;background-color: #cccccc">
                           	    ...
                           	    <table class="table table-striped" id="table">
                                    ...
                                    <tbody id="body">
                                        @foreach($album as $album)
                                   	    ...
                                   	    <div>
                                   	    	<a href="album/{{$album->title}}"><img src="{{$album->image}}" style="width: 75px"></a>
                                   	    </div>
                                   	    <div style="padding-left: 15px;width: 300px">
                                   	    	<div style="margin-top: 15px">
                                   	    		<a href="album/{{$album->title}}" style="font-size: 1em!important">{{$album->name}}</a>
                                   	    	</div>
                                   	    	<div>
                                   	    		@for($i=0;$i<count($album->album_singer);$i++)
													@if($i==0) 
													<a href="nghe-si/{{$album->album_singer[$i]->singer->title}}">{{$album->album_singer[$i]->singer->name}}</a>
													@else
													, <a href="nghe-si/{{$album->album_singer[$i]->singer->title}}">{{$album->album_singer[$i]->singer->name}}</a>
													@endif
												@endfor
										...
										<div style="display: flex;padding:6px 20px 11px 20px;">
											...
											<div >
												...
												@for($i=0;$i<count($song->music_singer);$i++)
													@if($i==0) 
													<a href="nghe-si/{{$song->music_singer[$i]->singer->title}}" style="display:inline;font-size:1em!important" href="">{{$song->music_singer[$i]->singer->name}}</a>
													@else
													, <a href="nghe-si/{{$song->music_singer[$i]->singer->title}}" style="display:inline;font-size:1em!important" href="">{{$song->music_singer[$i]->singer->name}}</a>
													@endif
												@endfor
											...
											<li>
												<div style="display: flex;padding:6px 20px 11px 20px;">
													...
													<div >
														...
														@for($i=0;$i<count($album->album_singer);$i++)
															@if($i==0) 
															<a href="nghe-si/{{$album->album_singer[$i]->singer->title}}" style="display:inline;font-size:1em!important" href="">{{$album->album_singer[$i]->singer->name}}</a>
															@else
															, <a href="nghe-si/{{$album->album_singer[$i]->singer->title}}" style="display:inline;font-size:1em!important" href="">{{$album->album_singer[$i]->singer->name}}</a>
															@endif
														@endfor
