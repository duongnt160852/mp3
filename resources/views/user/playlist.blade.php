			...
			<div id="page-wrapper">
				<div class="inner-content">
					<div class="music-left">
						<h3><b>{{$playlist->name}}</b></h3>
						<div id="player1">
							<div class="album1">
								<div class="row" style="margin: 0px">
									<div class="col-md-4" id="image">
										<img id="img1"style="height: 225px;width: 225px;" src="{{$playlist->playlistMusic[0]->music->image}}">
									</div>
									<div class="col-md-7">
										<div class="currently-playing">
										<h2 class="song-name">{{$playlist->playlistMusic[0]->music->name}}</h2>
										<h3 class="artist-name">Ca sĩ: @for($j=0;$j<count($playlist->playlistMusic[0]->music->music_singer);$j++)
											@if($j+1==count($playlist->playlistMusic[0]->music->music_singer)) 
											<span>{{$playlist->playlistMusic[0]->music->music_singer[$j]->singer->name}}</span>
											@else
											<span>{{$playlist->playlistMusic[0]->music->music_singer[$j]->singer->name}},</span>
											@endif
										@endfor - Sáng tác: {{$playlist->playlistMusic[0]->music->musician}}</h3>
									</div>
									</div>
								</div>
							</div>
							<div class="info1">
								...
								<div class="down1" style="visibility: hidden;"><a href="{{$playlist->playlistMusic[0]->music->link}}" target="_self" download><i class="fas fa-download"></i></a></div>
							...
							<script>
								var link="{{$playlist->playlistMusic[0]->music->link}}";
								var count={{count($playlist->playlistMusic)}};
								var arr=[];
								current=0;
								shuffle=false;
							</script>
							...
							<?php 
								foreach($playlist->playlistMusic as $song){
									echo "<script>arr.push(".$song->music->id.")</script>";
								}
							?>
							<script>
								const undoRedoHandler = new UndoRedoHandler({
									  id : {{$playlist->playlistMusic[0]->music->id}},
									  index: 0
									});
							</script>
						...
						<div class="list1">
							@for($i=0;$i<count($playlist->playlistMusic);$i++)
                               <div class="song1" id="song1{{$playlist->playlistMusic[$i]->music->id}}">
                               	{{$i+1}}. <a style="display:inline;font-size:1em!important" id="{{$playlist->playlistMusic[$i]->music->id}}">{{$playlist->playlistMusic[$i]->music->name}}</a>
                               	<span>- @for($j=0;$j<count($playlist->playlistMusic[$i]->music->music_singer);$j++)
											@if($j+1==count($playlist->playlistMusic[$i]->music->music_singer)) 
											<a href="nghe-si/{{$playlist->playlistMusic[$i]->music->music_singer[$j]->singer->title}}" style="display:inline;font-size:0.8em!important">{{$playlist->playlistMusic[$i]->music->music_singer[$j]->singer->name}}</a>
											@else
											<a href="nghe-si/{{$playlist->playlistMusic[$i]->music->music_singer[$j]->singer->title}}" style="display:inline;font-size:0.8em!important">{{$playlist->playlistMusic[$i]->music->music_singer[$j]->singer->name}}</a>,
											@endif
										@endfor</span>
                               	<div class="control1">
                               		<span style="position: relative;right:100px;">{{$playlist->playlistMusic[$i]->music->views}}</span>
                               		<i style="position: relative;right:90px;" class="fas fa-headphones"></i>                            		
                               		<i class="fas fa-play play1"></i>
                               		<a href="{{$playlist->playlistMusic[$i]->music->link}}" target="_self" download><i class="fas fa-download download1"></i></a>
                               		<i class="fas fa-plus myBtn add1" id="song{{$playlist->playlistMusic[$i]->music->id}}"></i>
                               	</div>
                               </div>
							@endfor
						</div>	
						...
						<script>
							...
							$(".song1 a").click(function(){
								...
								var myElement = document.getElementById('song1'+$(this).attr('id'));
								var topPos = myElement.offsetTop;
								document.getElementsByClassName('list1').scrollTop = topPos;
								var a=$(this);
						...
						<div class="lyric">
							...
							<h3>Lời bài hát: {{$playlist->playlistMusic[0]->music->name}}</h3>
							<div class="content">{!! $playlist->playlistMusic[0]->music->lyric !!}</div>							 
						</div>
						<!--//End-albums-->
					</div>
					<!--//music-left-->
